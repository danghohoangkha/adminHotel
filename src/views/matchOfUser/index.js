...
import EnhancedTableHead from '../roomManagement/EnhanceTableHead'
...
export default function matchUser({location}) {
  ...
  React.useEffect(()=>{
    CallAPI(`getAllMatchOfUser/${location.match.params.id}`, 'GET', null).then(res =>{
      setRows(res.data);
      setIsLoad(false);
      console.log('12345');
    })
  ...
  return (
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        <EnhancedTableToolbar area = {location.area} onSetFilter={handleSetFilter} id={location.match.params.id}/>
        <TableContainer>
          <Table
