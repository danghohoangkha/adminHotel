...
export default function EnhancedTable(props) {
  ...
  React.useEffect( ()=>{
     CallAPI('getAllOrder', 'GET', null).then(res =>{
  ...
  const filterList = (newFilter) => {
    const textSearchRows = rows.filter(row => { 
      return row.HinhThucTT.toLowerCase().includes(newFilter.searchText.toLowerCase()) ||
        row.MaHD.toLowerCase().includes(newFilter.searchText.toLowerCase()) 
    })
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{row.HinhThucTT}</TableCell>
                      <TableCell align="left">{row.TongTienThu}</TableCell>
                      <TableCell align="left">{row.MaPDP}</TableCell> 
                      <TableCell align="left">{row.HoTen}</TableCell>                    
                      <TableCell>
                      <Link to = {`/Detail-Order/${row.MaHD}`} style = {{textDecoration: "none"}}>
